...
################ ALL MODES ##################
    ...
    def __init__(self, log_queue, prompt_queue, usergui_process, event_arr, static_info_dict):
        ...
        match static_info_dict["Mode"]: 
            case "Developer":    # handle dev mode 
                self.launch_dev_mode()
            case "Test":         # handle test mode
                pass
            case "User":         # handle user mode 
...
################ USER MODE ##################
    ...
    def launch_user_mode(self):
        ...
        self.after(0, self.check_for_logs, 2) 

    def check_for_logs(self, mode):
        ...
        while not self.log_queue.empty():
            ...
            if loggername not in self.log_src_arr: 
                ...
                match mode: 
                    ...
                    case _:
                        width = 10
                tk.Button(self.log_menu, text=self.log_src_arr[-1], height=4, width=width, command=lambda index=cur_idx: self.switch_text(index)).grid(row=cur_idx, column=0)
        ...
        self.after(100, self.check_for_logs, mode)  # check every 100ms
...
################ DEVELOPER MODE ##################
    ...
    def launch_dev_mode(self):
        ...
        self.prompt_title_frame = tk.Frame(self, bg="lightgrey", height=50, relief="ridge", bd=5)
        self.prompt_title_frame.grid(column=0, row=1, sticky="nsew")
        self.prompt_title_frame.grid_propagate(False)
        self.prompt_title_label = tk.Label(self.prompt_title_frame, bg="lightgrey", font=('Times New Roman', 17), text="Server Console")
        self.prompt_title_label.pack(side="top")

        self.command_frame = tk.Frame(self, height=245, bg="lightgrey", relief="ridge", bd=5)
        self.command_frame.grid(column=0, row=2, sticky="new")
        self.command_frame.pack_propagate(False)

        self.command_label = tk.Label(self.command_frame, bg="lightgrey", text="Enter Request to Server:", font=('Times New Roman', 24))
        self.command_label.pack(side="top")

        self.command_entry = tk.Entry(self.command_frame, font=('Times New Roman', 24), width=30)
        ...
        self.prompt_frame = tk.Frame(self, height=555, bg="lightgrey")
        self.prompt_frame.grid(column=0, row=2, sticky="new", pady=250)
        ...
        self.prompt_label = tk.Label(self.prompt_frame, font=('Times New Roman', 17), text="Server console", bg="lightgrey")
        self.prompt_label.grid(column=0, row=0, sticky="n", pady=10)
        self.prompt_text = tk.Text(self.prompt_frame, wrap=tk.WORD, bg="white", height = 8, width=5)
        ...
        self.prompt_entry = tk.Entry(self.prompt_frame, font=('Times New Roman', 16), width=30)
        ...
        # create log title frames
        self.log_title_frame = tk.Frame(self, bg="lightgrey")
        self.log_title_frame.grid(row=1, column=1, sticky="nsew")
        self.log_title_frame.grid_propagate(False)
        self.log_label_frame = tk.Frame(self.log_title_frame, bg="lightgrey")
        self.log_label_frame.pack(side="left")
        self.log_label = tk.Label(self.log_label_frame, text='Select Log Source:', bg="lightgrey", font=('Times New Roman', 17, 'bold'))
        self.log_label.pack(side="left")
        self.log_display_label = tk.Label(self.log_title_frame, bg="lightgrey", text='Aggregate Logs', font=('Times New Roman', 17, 'bold'))
        self.log_display_label.pack(side="top")
        
        # create main frame
        self.main_frame = tk.Frame(self, height=613, bg="lightgrey", relief="ridge", bd=5)
        self.main_frame.grid(row=2, column=1, sticky="nsew")
        self.main_frame.grid_propagate(False)

        # configure main frame grid
        for i in range(6):
            self.main_frame.columnconfigure(i, minsize=100) # divide into 6 200px columns
            if i==5: self.main_frame.columnconfigure(i, minsize=94)
        ...
        # create canvas object for menu 
        self.canvas = tk.Canvas(self.scrollbar_frame, height=600, width=98)
        self.canvas.pack_propagate(False)
        self.canvas.pack(side="left", fill="both", expand=True)
        ...
        # create "Aggregate" log button
        tk.Button(self.log_menu, text=self.log_src_arr[-1], height=4, width=10, command=lambda index=0: self.switch_text(index)).grid(row=0, column=0)
        self.log_menu.update_idletasks() # update frame with new button
        ...
        # check for logs/prompts and update GUI
        self.after(0, self.check_for_logs, 0) 
